...
class TestShallowDiffer(object):
    ...
    def test_diff(self):
        ...
        assert SHALLOW_DIFFER.diff({u'x': 4, u'y': [1, 2, 3]}, {u'x': 4, u'y': [1, 2, 3]}) == {}
        assert SHALLOW_DIFFER.diff({u'x': 4, u'y': [1, 2, 3]}, {u'x': 4, u'y': [1, 2, 6]}) == \
               {u'c': {u'y': [1, 2, 6]}}
        assert SHALLOW_DIFFER.diff({u'x': 8, u'y': [1, 2, 3]}, {u'x': u'8', u'y': [1, 2, 3]}) == \
               {u'c': {u'x': u'8'}}
        assert SHALLOW_DIFFER.diff({u'x': 8, u'y': [54, 2, 3]}, {u'x': u'8', u'y': [1, 2, 6]}) == \
               {u'c': {u'x': u'8', u'y': [1, 2, 6]}}
        assert SHALLOW_DIFFER.diff({u'x': 4, u'y': u'beans'}, {u'x': 4}) == {u'r': [u'y']}
        ...
        assert SHALLOW_DIFFER.diff({u'l': u'beans'}, {u'l': u'beans', u'x': 4, u'y': 12}) == \
               {u'c': {u'x': 4, u'y': 12}}
        assert SHALLOW_DIFFER.diff({u'x': 4, u'y': u'beans'}, {u'l': 24246, u'x': 5}) == \
    ...
    def test_patch(self):
        ...
        assert SHALLOW_DIFFER.patch({u'r': [u'x']}, {u'x': 2380, u'y': u'goat'}) == {u'y': u'goat'}
        with pytest.raises(KeyError):
            assert SHALLOW_DIFFER.patch({u'r': [u'x']}, {})
        assert SHALLOW_DIFFER.patch({u'r': [u'x'], u'c': {u'b': 2}}, {u'x': 2380}) == {u'b': 2}
